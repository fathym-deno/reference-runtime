...
import type { ExtractKeysByPrefix, IsObjectNotNative, RemoveIndexSignatures, ValueType } from "../.deps.ts";
...
export type FluentMethodsRecord<
  ...
  TBuilderModel,
> =
  & (RemoveIndexSignatures<T> extends infer U ? K extends keyof U ? true extends $FluentTagExtractValue<U[K], "Methods", "generic"> ? GenericMethod<T, K, TBuilderModel>
      : NonGenericMethod<T, K, TBuilderModel>
    : true extends $FluentTagExtractValue<T[K], "Methods", "generic"> ? GenericMethod<T, K, TBuilderModel>
    : NonGenericMethod<T, K, TBuilderModel>
    : never)
  & (ExtractKeysByPrefix<T[K], "$"> extends infer U ? SelectFluentMethods<U, TBuilderModel>
    : {})
...
> =
  ...
  & (true extends IsObjectNotNative<TMethods> ? true extends IsFluentRecord<TMethods> ? string extends keyof T[K] ? FluentMethodsRecord<T[K], string, TBuilderModel>
        //T[K]//FluentMethodsRecord<T[K], any, TBuilderModel>
      : SelectFluentMethods<TMethods, TBuilderModel>
      //true
    : SelectFluentMethods<TMethods, TBuilderModel>
    : string extends keyof T[K] ? FluentMethodsProperty<T[K], string, TBuilderModel>
    : never)
